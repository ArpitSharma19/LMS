...
export const getDashboardStats = catchAsync(async (req, res) => {
  ...
  const { data: revenueData } = await supabase.from('revenue_tracking').select('*').limit(1).single();
...
export const getAllUsers = catchAsync(async (req, res) => {
  ...
  if (role === 'educator') {
    filteredUsers = users.filter(u => u.educatorProfile);
...
export const getPendingEducators = catchAsync(async (req, res) => {
  const { data: educators, error } = await supabase
    .from('educators')
    .select('*, users(id, name, email, image_url, created_at)')
...
export const approveEducator = catchAsync(async (req, res) => {
  ...
  if (profile.users) sendEmail(profile.users.email, '🎉 Educator Account Approved!', getEducatorApprovedEmailTemplate(profile.users.name)).catch(() => {});
...
export const rejectEducator = catchAsync(async (req, res) => {
  ...
  if (profile.users) sendEmail(profile.users.email, 'Educator Application Update', getEducatorRejectedEmailTemplate(profile.users.name, reason)).catch(() => {});
...
export const updateCommissionSettings = catchAsync(async (req, res) => {
  ...
  if (settings) {
    const { data: updated } = await supabase.from('commission_settings').update(updateData).eq('id', settings.id).select().single();
    res.json({ success: true, data: updated });
  } else {
    const { data: created } = await supabase.from('commission_settings').insert([updateData]).select().single();
